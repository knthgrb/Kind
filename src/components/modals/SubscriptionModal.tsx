        ...
        {currentSubscription && (
          <div className="p-6 bg-blue-50 border-b border-gray-200">
            <div className="flex items-center gap-3 mb-3">
              {getTierIcon(
                currentSubscription.subscription_plans?.tier || "free"
              )}
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  Current Plan:{" "}
                  {currentSubscription.subscription_plans?.name || "Unknown"}
                </h3>
                <p className="text-sm text-gray-600">
                  {currentSubscription.subscription_plans?.tier?.toUpperCase()}{" "}
                  •
                  {currentSubscription.subscription_plans
                    ?.swipe_credits_monthly === -1
                    ? " Unlimited"
                    : ` ${currentSubscription.subscription_plans?.swipe_credits_monthly}`}{" "}
                  swipe credits per day
                </p>
              </div>
            </div>

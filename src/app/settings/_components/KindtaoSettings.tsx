...
const JobPreferencesModal = dynamic(
  () => import("@/components/modals/JobPreferencesModal"),
  { ssr: false }
);

export default function KindtaoSettings() {
  ...
  // Check if user is kindtao
  const isKindtao = (user?.user_metadata as any)?.role === "kindtao";

  // Load job preferences and subscription data
  useEffect(() => {
    if (isKindtao) {
      loadJobPreferences();
    }
  ...
  const handleJobPreferencesSave = async (preferences: JobPreferences) => {
    try {
      const result = await JobPreferencesService.updateJobPreferences(
        preferences
      );
      if (result.success) {
        setJobPreferences(preferences);
        showSuccess("Success", "Job preferences updated successfully");
        setIsJobPreferencesModalOpen(false);
      } else {
        showError("Error", result.error || "Failed to update job preferences");
      }
    } catch (error) {
      console.error("Error saving job preferences:", error);
      showError("Error", "Failed to update job preferences");
    }
  };
  ...
  return (
    ...
    >
      ...
      <SubscriptionModal
        ...
        userRole={user?.user_metadata?.role || "kindtao"}

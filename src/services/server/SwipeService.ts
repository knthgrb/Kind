...
export class SwipeService {
  /**
   * Get user's current swipe credits status (server-side)
   */
   ...
   */
  static async getSwipeLimitStatusClient(
    userId: string
    ...
    const lastActiveDate = data.last_active
      ? new Date(data.last_active).toISOString().split("T")[0]
      : null;
  ...
  static async consumeSwipeCreditServer(userId: string): Promise<SwipeResult> {
    ...
    // Decrement the credit (only if not unlimited)
